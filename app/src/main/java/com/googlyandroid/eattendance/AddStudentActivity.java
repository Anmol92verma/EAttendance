...
public class AddStudentActivity extends AppCompatActivity {
  ...
  public void fnRegister(View view) {
    ...
    for (int i = 0; i < 8; i++)
      subject.add(0);
    if (!cb1.isChecked()) subject.add(0, -1);
    if (!cb2.isChecked()) subject.add(1, -1);
    if (!cb3.isChecked()) subject.add(2, -1);
    if (!cb4.isChecked()) subject.add(3, -1);
    if (!cb5.isChecked()) subject.add(4, -1);
    if (!cb6.isChecked()) subject.add(5, -1);
    if (!cb7.isChecked()) subject.add(6, -1);
    if (!cb8.isChecked()) subject.add(7, -1);
    int sum = 0;
    for (int i = 0; i < 8; i++)
    ...
    if (password.length() < 6) {
      Toast.makeText(AddStudentActivity.this, "Password should contain min. 6 characters",
    ...
    try {
      final StudentInformation stdinfo =
          new StudentInformation(name, rollno, dept, phone, email, password, subject);
      firebaseAuth.createUserWithEmailAndPassword(email, password)
          .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            @Override public void onComplete(Task<AuthResult> task) {
              if (task.isSuccessful()) {
                Toast.makeText(AddStudentActivity.this, "User Registered Successfully",
                    Toast.LENGTH_SHORT).show();
                saveInformation(stdinfo);
                pd.dismiss();
              } else {
                Toast.makeText(AddStudentActivity.this, "Unable to register", Toast.LENGTH_SHORT)
                    .show();
